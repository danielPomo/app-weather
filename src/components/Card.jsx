  ...
  return (
    <div className="weather">
      ...
      <div className={isDark ? "card dark-card" : "card light-card"}>
        <p>
          <span className="card__temp t1">
            {" "}
            {isFarenheit
              ? `${parseInt((location.main?.temp * 9) / 5 - 459)} `
              : `${parseInt(location.main?.temp) - 273} `}{" "}
          </span>
          <span className="card__temp--unit">{isFarenheit ? "F" : "°C"}</span>
        </p>
        <span className="card__wind t3">
          Winds: {location.wind?.speed} m/s{" "}
        </span>
        <span className="card__cloud2t3">
          Cloudiness: {location.clouds?.all} %
        </span>
        <span className="card__pressu2e t3">
          Pressure: {location.main?.pressure} hPa
        </span>
        ...
        <img
          ...
          src={icons[location.weather?.[0].icon]}
          alt="icono de clima"
        />
      </div>
      <button
      ...
      >
        {(isFarenheit) ? "Convert to °C" : "Convert to F"}
      </button>
    </div>
  );
};
